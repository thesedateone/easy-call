...
class ResultInline(admin.TabularInline):
    ...
    def formfield_for_choice_field(self, db_field, request, **kwargs):
        ...
        return super(ResultInline, self).formfield_for_choice_field(
...
class CallInline(admin.StackedInline):
    ...
    fields = (
        ...
        'duration_output',
        ('data1_output', 'data1_display', 'data1_addon'),
        ('data2_output', 'data2_display', 'data2_addon'),
        ('data3_output', 'data3_display', 'data3_addon'),
        ('data4_output', 'data4_display', 'data4_addon'),
        ('data5_output', 'data5_display', 'data5_addon'),
        ('data6_output', 'data6_display', 'data6_addon'),
        ('data7_output', 'data7_display', 'data7_addon'),
        ('data8_output', 'data8_display', 'data8_addon'))
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        return super(CallInline, self).formfield_for_foreignkey(
                                            db_field, request, **kwargs)
...
class ExtraInline(admin.StackedInline):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name.endswith('_source'):
            list_type = request.META['PATH_INFO'].strip('/').split('/')[-1]
            kwargs["queryset"] = CsvColumn.objects.filter(list_type=list_type)
        return super(ExtraInline, self).formfield_for_foreignkey(
                                            db_field, request, **kwargs)
...
class SystemNoteInline(admin.StackedInline):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name.endswith('_source'):
            list_type = request.META['PATH_INFO'].strip('/').split('/')[-1]
            kwargs["queryset"] = CsvColumn.objects.filter(list_type=list_type)
        return super(SystemNoteInline, self).formfield_for_foreignkey(
                                                db_field, request, **kwargs)
...
class DemographicsInline(admin.StackedInline):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name.endswith('_source'):
            list_type = request.META['PATH_INFO'].strip('/').split('/')[-1]
            kwargs["queryset"] = CsvColumn.objects.filter(list_type=list_type)
        return super(DemographicsInline, self).formfield_for_foreignkey(
                                                db_field, request, **kwargs)


class ListTypeAdmin(admin.ModelAdmin):
    inlines = [ResultInline, CallInline,
               DemographicsInline, SystemNoteInline, ExtraInline]
